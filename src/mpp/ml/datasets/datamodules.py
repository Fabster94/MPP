...
class MPP_datamodule(pl.LightningDataModule):
    ...
    """
    def __init__(self, batch_size=32, num_workers=0, input_type="vecset", target_type="seq", dataset="tkms"):
        super().__init__()
...
if __name__ == "__main__":
    ...
    for input_type in ["vecset"]:
        for target_type in [ "step-set", "seq"]:
            vecset_data_module = MPP_datamodule(batch_size=32, num_workers=0, input_type=input_type, target_type=target_type, dataset="tkms")
            vecset_data_module.setup(stage="fit")
